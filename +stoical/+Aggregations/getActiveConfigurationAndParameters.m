...
function [ thisActiveSetting ] = getActiveConfigurationAndParameters( systemname, STOICAL )
    ...
    % ------------- Get Active Variant Set -------------
        thisActiveSetting.ActiveVariantSet = stoical.VariantSubsystems.baseworkspaceoperations.getActiveVariantSetFromBaseWorkspace( STOICAL.VariantDefinition );

    % ------------- Retrieve Current Parameters ------------- 
        thisActiveSetting.ActiveParametersSet = stoical.Parameters.getActiveParameterValuesFromModelWorkspace( systemname, STOICAL.ParameterDefinition );

    % ------------- Get Active Simulink Model Configuration -------------   
        thisActiveSetting.ActiveSimulinkConfigurationSet = stoical.ModelConfiguration.getSimulinkModelConfiguration( systemname );

    % ------------- Get Active SimPowerSystems 2nd Generation Configuration -------------   
        thisActiveSetting.ActiveSimPowerSystemsConfigSet = stoical.ModelConfiguration.getSimPowerSystemsV2Configuration( systemname );
