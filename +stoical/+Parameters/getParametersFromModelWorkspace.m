...
%% Update Handles    
    [named_params.ParentalBlockHandle,hdlsnotfound,named_params.ParentalBlockPath] = stoical.InternalDataStorage.getUpdatedBlkHandles(named_params.ParentalBlockPath);
    if ~ignoreThoseNotFound && any(hdlsnotfound)
        error('STOICAL: Structure of model seems to have changed since model preparation !!');
    end    

    [named_params.BlkHandle,hdlsnotfound,named_params.BlkPath] = stoical.InternalDataStorage.getUpdatedBlkHandles(named_params.BlkPath);
    if ~ignoreThoseNotFound && any(hdlsnotfound)
        error('STOICAL: Structure of model seems to have changed since model preparation !!');
    end    
