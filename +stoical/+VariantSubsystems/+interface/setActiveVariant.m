...
%%    
    ...
    [~, VShdl2set] = stoical.Label.Paths.l2p(varsubsys_labelpath_regexp,...
                         STOICAL.VariantDefinition.NodeHandle(filt,:),...
                         'multiple', p.Results.multiple);
                     
    % Setze die Variablen
    for VShdl = reshape(VShdl2set,1,[])
        [ var2set, value2set ] = stoical.VariantSubsystems.get.getVSExplVarSettingforVariant( VShdl, variant_regexp, STOICAL.VariantDefinition, ' ' );
