...
%% Finde die pot. Log-Signale
    named_signals = stoical.Signals.get.getListSignalsWithRegExpName(system2load,SIGNAL_RegExp);
    
%% Schreibe die Variablen in die Log-Signal Namen    
    named_signals = stoical.Signals.set.setSignalsExplVars2Model( named_signals );
...
%% Ermittel labelpath der Signale
    ...
    if named_signals.NrSig > 0
        ...
        for i = 1:size(blkpath)
            [ labelstring{i,1} ] = stoical.Label.Paths.path2labelstring( blkpath{i,1}, RegExprForSignalLabeling);
        end
    else
        labelstring = {''};
    end
...
%% Save 2 Model    
    stoical.Signals.modelworkspaceoperations.writeSignalsToModelWorkspace( named_signals, system2load, 'STOICAL' );
