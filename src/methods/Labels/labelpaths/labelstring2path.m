    ...
    if hasraute(1) == 1
        ...
        if foundposofraute(1) ~= 1
            ...
            if isempty(foundidx)
                blkpath = '';
                blkhdl  = [];
                warning([getToolboxName() ': No correspondig element found for given labelpath "' labelstring '" -> ignoring']);
            else
                if length(foundidx) == 1
                    ...
                    if p.Results.multiple == false
                        warning([getToolboxName() ': Set "multiple" to true if nescessary']);
                        error([getToolboxName() 'Given labelstring does not resolve to unique path']);
                    else
                        blkpath = NodePaths(foundidx);
                        blkhdl  = BlkHandles(foundidx);
                    end
