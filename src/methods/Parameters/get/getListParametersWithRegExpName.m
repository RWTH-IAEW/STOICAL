...
%% Check if subsystems with parameters defined have corresponding mask values
    ...
    for iParamSubsys = reshape(find(filt_subsystems),1,[])
        ...
        SpecialParams = getSpecialSubsystemParameters(SubHdl);
        ...
        if no_mask
            error([getToolboxName() ': No Subsystem Mask found for Subsystem with defined Parameters: "' named_params.BlkPath{iParamSubsys} '" ! -> Check & Repeat Activation']);
        else
            if ~is_Param_inMask 
                error([getToolboxName() ': Parameter "' named_params.ParamName{iParamSubsys} ...
