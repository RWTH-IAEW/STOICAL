...
function [ unnamed_signals ] = getUnnamedLoggedSignals( str_model )
...
unnamed_signals = struct();

unnamed_signals.ParentalBlockPath = ParentBlocksOfSignals(idxLinelogbutnosig);
%unnamed_signals.NameInDiagram     = SignalNamesOnce;
unnamed_signals.SourceBlockPath   = StartingBlockOnce(idxLinelogbutnosig);
unnamed_signals.SourceBlockPort   = cell2mat(PortNrs(idxLinelogbutnosig));
unnamed_signals.Handle            = HandlesOnce(idxLinelogbutnosig);
