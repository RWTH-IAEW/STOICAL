...
%% Update Handles    
    ...
    [named_signals.ParentalBlockHandle,hdlsnotfound,named_signals.ParentalBlockPath] = getUpdatedBlkHandles(named_signals.ParentalBlockPath);
    if any(hdlsnotfound)
        error('STOICAL: Structure of model seems to have changed since model preparation !!');
    end    

    [named_signals.SourceBlockHandle,hdlsnotfound,named_signals.SourceBlockPath] = getUpdatedBlkHandles(named_signals.SourceBlockPath);
    if any(hdlsnotfound)
        error('STOICAL: Structure of model seems to have changed since model preparation !!');
    end    
