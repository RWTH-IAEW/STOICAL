...
named_params = getListParametersWithRegExpName(system2check,PARAM_RegExp);

named_params = setDefaultParamFieldEntries( named_params );

named_params = setExplVarNamesForParams(named_params);
...
%% Define Labelpath for Parameters
    ...
    if named_params.NrParams > 0
        ...
        named_params.LabelPath = cellfun(@(x) path2labelstring(x,RegExprForParameterLabeling),...
...
%% Lets see if there are identical Labelpath ... e.g. across Variants
    ...
    map = mapNonuniqueAtoUniqueB(named_params.LabelPath,uniqueLabelPath);
...
%% Define Default Parameter Values
    ...
    while ParamsDone < nrOfUniqueParams
        ...
        uniqueParamsValues2set(From:To) = answer;
...
%% Write Variables to Block Diagram
    named_params = setModelParameters2ExplVar(named_params);

%% Save Lists 2 Model
    writeParametersToModelWorkspace(named_params, system2load, 'STOICAL' );
